# MP3-Gotchi firmware v0.1.9

This is the MP3-Gotchi 0.1.9 raw progress firmware. It keeps the working YX5300 UART pinout, uses YX5300 module-relative next/previous commands for normal navigation, fixes MPU6050 handling to match the confirmed standalone test sketch, uses a generated bitmap font from the supplied real Aseprite Mini OTF asset, adds user-generated emotes, and keeps user-facing settings in one community-friendly file.
...
- MPU diagnostics default to the confirmed `SDA GPIO15 / SCL GPIO5` wiring, then can still test swapped pins and I2C speeds.
...
```powershell
.\.venv\Scripts\python.exe .\firmware\tools\generate_aseprite_mini_font.py --input C:\path\to\aseprite-mini.otf --output .\firmware\src\mp3gotchi-0.1.9\AsepriteMiniFontData.h --font-size 5
```
...
`mpu sample` prints one accelerometer, gyro, temperature, totalG, and shake sample using the same units as the standalone test sketch.
...
If an older firmware saved an invalid track number, `0.1.9` tries to repair it when the module answers the total-track query.

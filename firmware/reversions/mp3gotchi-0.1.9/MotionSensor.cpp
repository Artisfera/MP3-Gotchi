...
bool MotionSensor::update() {
  ...
  bool strongAccel =
    fabsf(accelTotalG - 1.0f) > Config::SHAKE_ACCEL_TOTAL_G_THRESHOLD ||
    accelChangeG > Config::SHAKE_ACCEL_CHANGE_G_THRESHOLD;
  bool strongGyro =
    fabsf(gx) > Config::SHAKE_GYRO_DPS_THRESHOLD ||
    fabsf(gy) > Config::SHAKE_GYRO_DPS_THRESHOLD ||
    fabsf(gz) > Config::SHAKE_GYRO_DPS_THRESHOLD;

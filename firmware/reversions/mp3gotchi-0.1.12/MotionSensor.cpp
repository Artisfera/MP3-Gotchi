...
bool MotionSensor::update(uint8_t sensitivity) {
  ...
  float level = (float)(sensitivity - 1) / 99.0f;
  float accelTotalThreshold = 1.25f - (level * 1.13f);
  float accelChangeThreshold = 0.85f - (level * 0.81f);
  float gyroThreshold = 720.0f - (level * 660.0f);
  unsigned long cooldownMs = (unsigned long)(900.0f - (level * 500.0f));

  if ((now - lastShakeMs) < cooldownMs) return false;

  bool strongAccel =
    fabsf(accelTotalG - 1.0f) > accelTotalThreshold ||
    accelChangeG > accelChangeThreshold;
  bool strongGyro =
    fabsf(gx) > gyroThreshold ||
    fabsf(gy) > gyroThreshold ||
    fabsf(gz) > gyroThreshold;

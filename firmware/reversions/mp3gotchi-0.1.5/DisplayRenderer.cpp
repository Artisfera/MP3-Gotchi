...
void DisplayRenderer::setBrightness(uint8_t percent) {
  ...
  uint8_t master = scaleByte(0x06, percent);
  uint8_t contrastA = scaleByte(0x91, percent);
  uint8_t contrastB = scaleByte(0x50, percent);
  uint8_t contrastC = scaleByte(0x7D, percent);

  display.sendCommand(SSD1331_CMD_MASTERCURRENT, &master, 1);
  display.sendCommand(SSD1331_CMD_CONTRASTA, &contrastA, 1);
  display.sendCommand(SSD1331_CMD_CONTRASTB, &contrastB, 1);
  display.sendCommand(SSD1331_CMD_CONTRASTC, &contrastC, 1);

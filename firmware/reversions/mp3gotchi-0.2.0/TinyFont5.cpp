...
void TinyFont5::renderTextMask(const char* text, int16_t x, uint8_t* mask, uint8_t width, uint8_t height) {
  ...
  while (*p) {
    ...
    if (cursorX + glyph.width > 0) {
      for (uint8_t yy = 0; yy < ASEPRITE_MINI_FONT_HEIGHT && yy < height; yy++) {
        uint8_t row = glyph.rows[yy];
        for (uint8_t xx = 0; xx < glyph.width; xx++) {
          int16_t px = cursorX + xx;
          if (px < 0 || px >= width) continue;
          if (row & (1 << (glyph.width - 1 - xx))) {
            mask[(uint16_t)yy * width + (uint8_t)px] = 1;
          }
        }
      }
    }

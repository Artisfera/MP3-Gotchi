# MP3-Gotchi firmware v0.2.0

This is the MP3-Gotchi 0.2.0 community firmware release. It keeps the working YX5300 and MPU6050 pinouts, keeps logical YX5300 track mapping, replaces the old artificial shake sensitivity scale with real MPU6050 thresholds, uses software OLED brightness scaling, and keeps user-facing settings in one community-friendly file.
...
- Serial command `shake` prints MPU6050 shake settings,
- Serial commands `shake hard`, `shake normal`, `shake light`, `shake verylight`, `shake off`, and `shake on` change and save shake profiles,
- Serial commands `shake total`, `shake change`, `shake gyro`, `shake confirm`, and `shake cooldown` fine tune shake detection and save it,
...
```powershell
.\.venv\Scripts\python.exe .\firmware\tools\generate_aseprite_mini_font.py --input C:\path\to\aseprite-mini.otf --output .\firmware\src\mp3gotchi-0.2.0\AsepriteMiniFontData.h --font-size 5
```
...
`shake` prints the current MPU6050 shake thresholds and command help.
The named shake profiles persist to flash. `hard` needs a very deliberate shake, `normal` is the default, `light` is easier, and `verylight` is intentionally sensitive.
Custom shake commands persist too:

- `shake total <g>` sets how far total acceleration must move away from normal gravity,
- `shake change <g>` sets the required frame-to-frame acceleration change,
- `shake gyro <dps>` sets the required rotation speed,
- `shake confirm <1-10>` sets how many consecutive strong samples are required,
- `shake cooldown <ms>` sets the delay after a detected shake.
...
This project is community-source and source-available for personal, educational, repair, modification, and non-commercial collaboration.

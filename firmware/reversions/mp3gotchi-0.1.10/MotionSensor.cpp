...
bool MotionSensor::update() {
  ...
  if ((now - lastShakeMs) < UserConfig::SHAKE_COOLDOWN_MS) return false;

  bool strongAccel =
    fabsf(accelTotalG - 1.0f) > UserConfig::SHAKE_ACCEL_TOTAL_G_THRESHOLD ||
    accelChangeG > UserConfig::SHAKE_ACCEL_CHANGE_G_THRESHOLD;
  bool strongGyro =
    fabsf(gx) > UserConfig::SHAKE_GYRO_DPS_THRESHOLD ||
    fabsf(gy) > UserConfig::SHAKE_GYRO_DPS_THRESHOLD ||
    fabsf(gz) > UserConfig::SHAKE_GYRO_DPS_THRESHOLD;

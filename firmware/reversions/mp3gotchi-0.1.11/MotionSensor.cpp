...
bool MotionSensor::update(uint8_t sensitivity) {
  ...
  float accelTotalThreshold = 0.65f - (level * 0.55f);
  float accelChangeThreshold = 0.30f - (level * 0.26f);
  float gyroThreshold = 220.0f - (level * 180.0f);
  unsigned long cooldownMs = (unsigned long)(650.0f - (level * 400.0f));

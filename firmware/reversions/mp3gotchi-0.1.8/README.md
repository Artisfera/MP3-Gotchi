...
```powershell
.\.venv\Scripts\python.exe .\firmware\tools\generate_aseprite_mini_font.py --input C:\path\to\aseprite-mini.otf --output .\firmware\src\mp3gotchi-0.1.8\AsepriteMiniFontData.h --font-size 5
```
...
If an older firmware saved an invalid track number, `0.1.8` tries to repair it when the module answers the total-track query.

...
void App::begin() {
  ...
  Serial.println();
  Serial.println("MP3-Gotchi firmware v0.1.8");
  Serial.println("Code created with ChatGPT as programming assistant. Concept and hardware by Patryk Ankudowicz (Artisfera).");
  Serial.println("Temporary debug commands: pinout, brightness 0-100, mpu diag, mpu scan, mpu pins <sda> <scl>, yx query, odwroc mpu, odwroc yx.");
...
void App::handleSerialCommands() {
  // TODO(v0.1.8): remove these temporary hardware debug commands after wiring is confirmed.
...
void App::processSerialCommand(char* command) {
  ...
  if (strcmp(command, "pinout") == 0) {
    ...
    Serial.println("Unknown command. Use: pinout, brightness 0-100, mpu diag, mpu scan, mpu pins <sda> <scl>, yx query, odwroc mpu, odwroc yx.");
  }
}

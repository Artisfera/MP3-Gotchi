...
public:
  struct ShakeSettings {
    bool enabled = true;
    float accelTotalG = 0.85f;
    float accelChangeG = 0.50f;
    float gyroDps = 360.0f;
    uint8_t confirmSamples = 2;
    unsigned long cooldownMs = 900;
  };

  bool begin(uint8_t sdaPin, uint8_t sclPin, uint32_t frequency = 100000);
  bool update(const ShakeSettings& settings);
  bool printSample();
...
private:
  ...
  uint8_t shakeCandidateSamples = 0;

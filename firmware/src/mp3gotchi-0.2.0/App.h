...
private:
  ...
  unsigned long lastTrackFinishedEventMs = 0;
  ...
  MotionSensor::ShakeSettings currentShake;
  uint16_t lastFinishedModuleTrack = 0;
  ...
  void printShakeSettings() const;
  void runMpuDiagnostics();
  void runYxDiagnostics();
  void setBrightness(uint8_t brightness);
  void loadShakeSettings();
  void saveShakeSettings();
  void applyShakeProfile(const char* profile);
  void setShakeSetting(char* command);
  void initMotion();
